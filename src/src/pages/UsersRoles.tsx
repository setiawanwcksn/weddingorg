...
const UsersRoles: React.FC = () => {
  ...
  const handlePhotoUpload = async (file: File, fieldName: 'weddingPhotoUrl' | 'weddingPhotoUrl_dashboard' | 'weddingPhotoUrl_welcome') => {
    const setUploading = fieldName === 'weddingPhotoUrl' ? setUploadingPhoto : 
                        fieldName === 'weddingPhotoUrl_dashboard' ? setUploadingDashboardPhoto : setUploadingWelcomePhoto;
    
    setUploading(true);
    try {
      console.log(`[UsersRoles] Starting photo upload for ${fieldName}:`, file.name, file.size, file.type);
      
      // Use the username from the form data (for new user creation) or selected user (for editing)
      const username = formData.username || selectedUser?.username;
      if (!username) {
        throw new Error('Username is required for photo upload');
      }
      
      // Create FormData with username for filename generation
      const uploadFormData = new FormData();
      uploadFormData.append('photo', file);
      uploadFormData.append('type', 'user');
      uploadFormData.append('username', username); // Pass username for filename generation
      
      // Add field type for proper filename generation
      if (fieldName === 'weddingPhotoUrl_dashboard') {
        uploadFormData.append('fieldType', 'weddingPhotoUrl_dashboard');
      } else if (fieldName === 'weddingPhotoUrl_welcome') {
        uploadFormData.append('fieldType', 'weddingPhotoUrl_welcome');
      } else {
        uploadFormData.append('fieldType', 'weddingPhotoUrl');
      }
      ...
      console.log(`[UsersRoles] Upload response status for ${fieldName}:`, response.status);
      
      if (!response.ok) {
        const errorText = await response.text();
        console.error(`[UsersRoles] Upload failed for ${fieldName}:`, errorText);
        throw new Error(`Failed to upload photo: ${errorText}`);
      }

      const result = await response.json();
      console.log(`[UsersRoles] Upload result for ${fieldName}:`, result);
      
      if (result.success) {
        setFormData({ ...formData, [fieldName]: result.data.url });
        showToast('Photo uploaded successfully', 'success');
      } else {
        throw new Error(result.error || 'Failed to upload photo');
      }
    } catch (error) {
      console.error(`[UsersRoles] Error uploading photo for ${fieldName}:`, error);
      showToast((error && typeof error === 'object' && 'message' in error) ? String(error.message) : 'Failed to upload photo', 'error');
    } finally {
      setUploading(false);
    }
  };
  ...
  const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      // Use user type for main photo
      handlePhotoUpload(file, 'weddingPhotoUrl');
    }
  };
  ...
  const handleDashboardPhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      // Use user_dashboard type for dashboard photo
      handlePhotoUpload(file, 'weddingPhotoUrl_dashboard');
    }
  };
  ...
  const handleWelcomePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      setSelectedWelcomePhoto(file);
      // Use user_welcome type for welcome photo
      handlePhotoUpload(file, 'weddingPhotoUrl_welcome');
    }
  };
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    if (!formData.weddingTitle.trim()) {
      showToast('Wedding title is required', 'error');
      return;
    }
    if (!formData.weddingDateTime) {
      showToast('Wedding date and time is required', 'error');
      return;
    }
    if (!formData.weddingLocation.trim()) {
      showToast('Wedding location is required', 'error');
      return;
    }
    
    // No need to validate permissions - users can be created without any page access
    
    try {
      // Send as JSON instead of FormData for better reliability
      const requestData = {
        ...
        weddingDateTime: formData.weddingDateTime ? new Date(formData.weddingDateTime).toISOString() : '',
        weddingLocation: formData.weddingLocation.trim(),
        weddingPhotoUrl: formData.weddingPhotoUrl.trim(),
        weddingPhotoUrl_dashboard: formData.weddingPhotoUrl_dashboard.trim(),
        weddingPhotoUrl_welcome: formData.weddingPhotoUrl_welcome.trim(),
        permissions: formData.role === 'user' ? permissionsData : [], // Include permissions only for user role
      };
      ...
      if (!response.ok) {
        ...
        try {
          ...
          // If we can't get the text at all, use default message
      ...
      if (result.success) {
        ...
        setPermissionsData([]); // Reset permissions
      ...
      showToast((error && typeof error === 'object' && 'message' in error) ? String(error.message) : 'Failed to create user', 'error');
    }
  };
  ...
  const handleUpdateUser = async (e: React.FormEvent) => {
    ...
    try {
      const updateData = {
        ...
        // Include wedding details in update
        weddingTitle: formData.weddingTitle.trim(),
        weddingDateTime: formData.weddingDateTime ? new Date(formData.weddingDateTime).toISOString() : '',
        weddingLocation: formData.weddingLocation.trim(),
        weddingPhotoUrl: formData.weddingPhotoUrl.trim(),
      ...
      if (formData.password.trim()) {
        (updateData as any).password = formData.password.trim();
      }
      ...
      showToast((error && typeof error === 'object' && 'message' in error) ? String(error.message) : 'Failed to update user', 'error');
    }
  };
      ...
      {showEditModal && selectedUser && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-background rounded-xl p-4 sm:p-6 w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-semibold text-text mb-4">Edit User - {selectedUser.username}</h2>
            <form onSubmit={handleUpdateUser} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-text mb-2">New Password (leave empty to keep current)</label>
                <input
                  type="password"
                ...
                <div className="grid grid-cols-1 gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      {formData.weddingPhotoUrl_dashboard && (
                        <span className="text-sm text-gray-600 truncate max-w-32">
                          Current dashboard photo set
                        </span>
                      )}
                    </div>
                    {formData.weddingPhotoUrl_dashboard && (
                      <div className="mt-2">
                        <img 
                          src={formData.weddingPhotoUrl_dashboard} 
                          alt="Current dashboard photo" 
                          className="w-16 h-16 object-cover rounded-lg border border-border"
                        />
                      </div>
                    )}
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      {formData.weddingPhotoUrl_welcome && (
                        <span className="text-sm text-gray-600 truncate max-w-32">
                          Current welcome photo set
                        </span>
                      )}
                    </div>
                    {formData.weddingPhotoUrl_welcome && (
                      <div className="mt-2">
                        <img 
                          src={formData.weddingPhotoUrl_welcome} 
                          alt="Current welcome photo" 
                          className="w-16 h-16 object-cover rounded-lg border border-border"
                        />
                      </div>
                    )}
                    ...
                    {AVAILABLE_PAGES.map((page) => {
                      ...
                      return (
                        <div key={page.key} className="flex items-center justify-between p-2 rounded-lg hover:bg-secondary transition-colors">
                          <label htmlFor={`edit-permission-${page.key}`} className="text-sm font-medium text-text cursor-pointer">
                            {page.label}
                          </label>
                          <ToggleSwitch
